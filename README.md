...
<head>
    ...
    <script type="module">
        ...
        const menu = [
            { name: "SAUMON", price: 15.00, icon: "🍣" }, 
            { name: "POULET", price: 15.00, icon: "🍗" },
            { name: "VIANDE HACHÉE", price: 15.00, icon: "🥩" }, 
            { name: "THON", price: 15.00, icon: "🐟" },
            { name: "COCA", price: 2.50, icon: "🥤" },
            { name: "OASIS", price: 2.50, icon: "🧃" }
        ];

        const PAYMENT_METHODS = [
            { type: 'Carte', icon: '💳' },
            { type: 'Especes', icon: '💵' },
            { type: 'TicketResto', icon: '🎫' },
        ];
        ...
        function showPaymentConfirmationModal(total, paymentType, totalRemaining) {
            return new Promise((resolve) => {
                ...
                if (paymentType === 'Especes') {
                    titleElement.innerHTML = '💵 Paiement Espèces';
                    contentElement.innerHTML = `
                        <div class="text-center mb-6">
                            <div class="text-3xl font-bold text-green-600 mb-2">À encaisser: ${amountToPay.toFixed(2)}€</div>
                        </div>
                        <div class="mb-4">
                            <label class="block text-gray-700 font-bold mb-2">Montant reçu du client:</label>
                        ...
                        <div id="rendu-info" class="text-center mb-6 p-4 bg-gray-100 rounded-xl">
                            <div class="text-lg font-bold text-blue-600">À rendre:</div>
                            <div id="montant-rendu" class="text-2xl font-bold text-blue-800">0.00€</div>
                        </div>
                    ...
                    montantInput.oninput = () => {
                        ...
                        if (rendu >= 0) {
                            montantRendu.textContent = rendu.toFixed(2) + '€';
                        } else {
                            montantRendu.textContent = "Manque " + Math.abs(rendu).toFixed(2) + '€';
                        }
                    };
                    ...
                    const title = paymentType === 'Carte' ? '💳 Paiement Carte (Tap to Pay)' : '🎫 Paiement TR';
                    titleElement.innerHTML = title;
                    let contentHTML = `<div class="text-center mb-6"><div class="text-3xl font-bold text-blue-600 mb-4">Montant: ${amountToPay.toFixed(2)}€</div>`;
                    ...
                    if (paymentType === 'Carte') {
                        ...
                        contentHTML += `<div class="bg-blue-50 p-6 rounded-xl border-2 border-blue-200 mt-4"><div class="text-6xl mb-4">🎫</div><div>Valider le ticket resto.</div></div>`;
                    }
                    
                    contentHTML += `</div>
                        <div class="flex flex-col space-y-2">
                            <button id="payment-confirm-btn" class="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 rounded-xl font-bold hover:shadow-lg">✅ Confirmer Paiement</button>
                            <button id="payment-cancel-btn" class="w-full bg-red-500 hover:bg-red-600 text-white py-3 rounded-xl font-bold">❌ Annuler</button>
                        </div>`;
            ...
            list.innerHTML = currentPayments.map((p, index) => `
                <li class="flex justify-between items-center py-2 px-3 bg-white bg-opacity-20 rounded-lg mb-2">
                    ...
                    <div class="flex items-center space-x-3">
                        <span class="text-white font-bold">${p.amount.toFixed(2)}€</span>
                        <button onclick="window.removePayment(${index})" class="text-white bg-red-500 w-6 h-6 rounded-full flex items-center justify-center font-bold text-sm">✕</button>
                    </div>
                </li>
            `).join('');
            
            // Mise à jour automatique du champ montant
        ...
        window.addSplitPayment = async () => {
            ...
            if (amount <= 0.001) {
                showMessageModal("Attention", "Le montant doit être supérieur à 0.");
                return;
            }
            if (amount > remaining + 0.01) {
                showMessageModal("Erreur", `Le montant (${amount}€) est supérieur au reste à payer (${remaining.toFixed(2)}€).`);
                return;
            }
        ...
        window.finalizeSplitPayment = async (docId) => {
            ...
            if (Math.abs(total - paid) > 0.01) {
                showMessageModal("Erreur", "Le total payé ne correspond pas au total de la commande.");
                return;
            }
        ...
        window.updateOrderStatus = async (docId, newStatus) => {
            ...
            if (confirm) {
                try {
                    ...
                    showMessageModal("Succès ✅", `Statut de la commande #${docId.substring(0, 8)} mis à jour à ${newStatus}.`);
                } catch(e) {
                    console.error("Erreur mise à jour statut: ", e);
                    showMessageModal("Erreur ❌", "Impossible de mettre à jour le statut.");
                }
            }
        };
        ...
        async function encaissementFinal(docId, payments) {
            ...
            try {
                const paymentSummary = payments.map(p => `${p.type}: ${p.amount.toFixed(2)}€`).join(' / ');
                ...
                showMessageModal("Succès ✅", "Commande encaissée avec succès !");
            } catch(e) {
                console.error("Erreur encaissement: ", e);
                showMessageModal("Erreur ❌", "Impossible d'encaisser la commande. " + e.message);
            }
        }
        
        window.encaisserCommande = async (docId, total) => {
            currentPayments = [];
            const confirm = await showConfirmModal("Encaissement", `Encaisser la commande pour un montant de ${total.toFixed(2)}€?`);
            if (!confirm) return;
        ...
        window.generateTicketPDF = async (docId) => {
            try {
                ...
                let articlesText = articlesList.map(item => `${item.quantity}x ${item.name} - ${(item.total_ligne || item.price * item.quantity).toFixed(2)}€` ).join('\n');
                
                if (orderData.remise_pourcentage > 0) {
                     const remiseAmount = orderData.total_avant_remise - orderData.total;
                     articlesText += `\nRemise ${orderData.remise_pourcentage}% (${orderData.code_promo}) : -${remiseAmount.toFixed(2)}€`;
                }
                
                let livraisonInfo = '';
                if (orderData.type_commande === 'LIVRAISON' && orderData.adresse_livraison) {
                    livraisonInfo = `\nLIVRAISON à: ${orderData.adresse_livraison}`;
                    if (orderData.frais_livraison > 0) {
                        livraisonInfo += `\nFrais: ${orderData.frais_livraison.toFixed(2)}€`;
                    }
                } else if (orderData.date_retrait && orderData.heure_retrait) {
                     livraisonInfo = `\nRETRAIT: ${orderData.date_retrait} à ${orderData.heure_retrait}`;
                }

                const ticketContent = `
                    PIZZA DJOUDI
                    19 rue François Plasson
                    -------------------------
                    COMMANDE #${orderData.numero_commande}
                    Client: ${orderData.client_prenom} ${orderData.client_nom}
                    Tél: ${orderData.client_telephone}
                    ${livraisonInfo} 
                    -------------------------
                    ${articlesText}
                    -------------------------
                    TOTAL PAYÉ: ${orderData.total.toFixed(2)}€
                ...
                console.error("Erreur génération ticket: ", e);
                showMessageModal("Erreur ❌", "Impossible de générer le ticket.");
            }
        };
        ...
        function handlePromoCode() {
            ...
            if (code === 'DJOUDI20') {
                ...
                showMessageModal("Succès 🎉", `Le code promo "${code}" a été appliqué (-20%) !`);
            } else if (code === '') {
                currentDiscountPercentage = 0;
                currentPromoCode = '';
                showMessageModal("Remise Annulée", "Le code promo a été retiré.");
            } else {
                currentDiscountPercentage = 0;
                currentPromoCode = '';
                showMessageModal("Erreur ❌", `Le code promo "${code}" est invalide.`);
            }
        ...
        function updateCaisseCartDisplay() {
            ...
            cartList.innerHTML = currentCart.map(item => `
                <li class="flex justify-between items-center py-2 px-3 bg-white bg-opacity-20 rounded-lg mb-2 slide-in">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="text-white ml-3 font-semibold w-16 text-right">${(item.price * item.quantity).toFixed(2)}€</span>
                    </div>
                </li>
            ` ).join('');
            ...
            if (currentDiscountPercentage > 0) {
                ...
                discountInfoElement.innerHTML = `
                    <div class="text-red-300 font-semibold mb-2">Code Promo ${currentPromoCode}: -${discountAmount.toFixed(2)}€ (-${currentDiscountPercentage}%)</div>
                `;
                discountInfoElement.classList.remove('hidden');
                cartTotal.textContent = totalAfterDiscountGlobal.toFixed(2) + '€';
            } else {
                totalBeforeDiscountElement.classList.add('hidden');
                discountInfoElement.classList.add('hidden');
                cartTotal.textContent = totalBeforeDiscount.toFixed(2) + '€';
            }
                ...
                if (!querySnapshot.empty) {
                    ...
                    infoDiv.innerHTML = `
                        <div class="text-center text-white font-bold">
                            Client trouvé: ${data.prenom} ${data.nom}
                        </div>
                        <div class="text-sm text-gray-300 mt-1">
                            Points Fidélité: ${data.points || 0} (Prochaine pizza: ${nextPizza} pts)
                        </div>
                    `;
        ...
        window.finalizeCaisseOrder = async () => {
            ...
            const confirm = await showConfirmModal("Finaliser Commande Caisse", `Confirmez la commande pour ${totalAfterDiscountGlobal.toFixed(2)}€ ?`);
            if (!confirm) return; 

            // IMPORTANT : On définit la source sur le modal, pas sur le panier
        ...
        window.finalizeCaissePaymentAndSave = async () => {
            ...
            try {
                ...
                const paymentSummary = currentPayments.map(p => `${p.type}: ${p.amount.toFixed(2)}€`).join(' / ');
                ...
                showMessageModal("Succès ✅", `Commande Caisse #${orderData.numero_commande} encaissée et enregistrée !`);
                window.clearCaisseCart();
                document.getElementById('caisse-finalize-btn').disabled = false;
                
            } catch(e) {
                console.error("Erreur finalisation commande caisse: ", e);
                showMessageModal("Erreur ❌", "Impossible d'enregistrer la commande. " + e.message);
                document.getElementById('caisse-finalize-btn').disabled = false;
            }
        };
        ...
        function renderOrder(c, container, showEncaissement) {
            let orderType = c.type_commande === 'LIVRAISON' ? '🏠 Livraison' : '🏃 Retrait';
            let orderIcon = c.type_commande === 'LIVRAISON' ? '🚚' : '📦';
            ...
            const ticketButton = `<button onclick="window.generateTicketPDF('${c.id}')" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-3 px-6 rounded-xl shadow-lg transform hover:scale-105 transition-all duration-200"> 📄 Ticket </button>`;
            ...
            if (c.type_commande === 'RETRAIT') {
                if (c.statut === 'EN_ATTENTE' || c.statut === 'EN_PREPARATION') {
                    ...
                    actionButtons = `
                        ...
                        <button onclick="window.updateOrderStatus('${c.id}', '${nextStatus}')" class="flex-1 bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg transform hover:scale-105 transition-all duration-200 neon-glow"> 
                            ${c.statut === 'EN_ATTENTE' ? '🔪 Préparer la Commande' : '🔔 Marquer "Prête"'}
                        </button>
                    `;
                    if (c.paiement === 'A_RECEPTION' && showEncaissement) {
                         actionButtons += `<button onclick="window.encaisserCommande('${c.id}', ${c.total})" class="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg transform hover:scale-105 transition-all duration-200 neon-glow"> 💰 Encaisser </button>`;
                    }
                } else if (c.statut === 'PAYEE') {
                    actionButtons = `
                        ${ticketButton}
                        <button onclick="window.updateOrderStatus('${c.id}', 'APPEL_RETRAIT')" class="flex-1 bg-gradient-to-r from-orange-400 to-red-500 hover:from-orange-500 hover:to-red-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg transform hover:scale-105 transition-all duration-200 neon-glow"> 
                            🗣️ Chercher la Commande #${c.numero_commande}
                        </button>
                    `;
                    ...
                    actionButtons = `
                        ...
                        <button disabled class="flex-1 bg-yellow-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg"> 
                            ⚠️ Commande #${c.numero_commande} en cours de ramassage... 
                        </button>
                    `;
                }
                ...
                if (c.statut === 'EN_PREPARATION') {
                    ...
                    if (c.paiement && c.paiement !== 'A_RECEPTION') {
                        actionButtons = `${ticketButton} <button disabled class="flex-1 bg-blue-600 text-white py-3 px-6 rounded-xl font-bold">En Livraison (Payée)</button>`;
                    } else {
                        actionButtons = `
                            ${ticketButton}
            ...
            if (c.type_source === 'CAISSE') {
                 actionButtons = `
                    ${ticketButton}
                    <button disabled class="flex-1 bg-gray-600 text-white py-3 px-6 rounded-xl font-bold">Commande Caisse (Déjà Payée)</button>
                 `;
            }
            ...
            card.innerHTML = `
                ...
                <p class="font-bold text-lg text-gray-800 flex items-center mb-2">
                    <i class="text-xl mr-2">👤</i> Client: ${clientName}
                </p>
                <p class="font-semibold text-gray-700 flex items-center mb-4">
                    <i class="text-xl mr-2">📞</i> Tél: ${c.client_telephone || 'N/A'}
                </p>
                ${c.type_commande === 'LIVRAISON' ? `
                    <div class="p-3 bg-red-50 border-l-4 border-red-500 rounded-lg mb-4">
                        <p class="font-semibold text-gray-700 flex items-start">
                            <i class="text-xl mr-2 mt-0.5">🏠</i> <span class="block">${c.adresse_livraison || 'Adresse non renseignée'}</span>
                        </p>
                    </div>
                ` : ''}
                    ...
                    <ul class="space-y-1 text-sm">
                        ${articlesList.map(item => `
                            <li class="flex justify-between text-gray-700">
                                ...
                                <span>${(item.total_ligne || item.price * item.quantity).toFixed(2)}€</span>
                            </li>
                        `).join('')}
                    </ul>
                    ${remiseAmount > 0 ? `
                        <div class="flex justify-between text-red-600 font-semibold border-t border-red-200 pt-2 mt-2">
                            ...
                            <span>-${remiseAmount.toFixed(2)}€</span>
                        </div>
                    ` : ''}
                </div>
                <div class="flex justify-between items-center bg-gray-200 p-3 rounded-xl mb-6">
                    ...
                    <span class="text-3xl font-black text-purple-600">${c.total.toFixed(2)}€</span>
                </div>
                <div class="text-sm font-semibold text-gray-600 mb-4">
                    Paiement: ${c.paiement_summary || c.paiement || 'A_RECEPTION'}
                    ${c.type_commande === 'LIVRAISON' && c.frais_livraison > 0 ? ` (+${c.frais_livraison.toFixed(2)}€ frais livr.)` : ''}
                </div>
        ...
        function renderCardOrder(c, container) {
            ...
            card.innerHTML = `
                <div class="flex justify-between items-center mb-4 border-b pb-3 border-gray-200">
                    <h3 class="text-xl font-bold text-purple-600"> 💳 Carte Cadeau #${c.numero_commande} </h3>
                ...
                <div class="p-3 bg-yellow-50 border-l-4 border-yellow-500 rounded-lg mb-4">
                    <p class="text-lg font-bold text-gray-700 flex items-center">
                        Montant: <span class="text-2xl font-black ml-2">${c.montant.toFixed(2)}€</span>
                    </p>
                </div>
                <div class="flex space-x-3">
        ...
        function renderAllTasks() {
            ...
            if (allTasks.length === 0) {
                 cuisineList.innerHTML = `<div class="text-center text-white opacity-60 py-8"><i class="text-6xl mb-4">🎉</i><p>Aucune commande ou carte en attente !</p></div>`;
                 return;
            }
            ...
            if (filteredTasks.length === 0) {
                cuisineList.innerHTML = `<div class="text-center text-white opacity-60 py-8"><i class="text-6xl mb-4">😴</i><p>Aucune commande en cours ne correspond à la recherche.</p></div>`;
                return;
            }
        ...
        window.updateCardStatus = async (docId, newStatus) => {
            ...
            if (confirm) {
                try {
                    await updateDoc(docRef, { statut: newStatus, date_statut_update: serverTimestamp() });
                    showMessageModal("Succès ✅", `Statut de la carte mis à jour à ${newStatus}.`);
                } catch(e) {
                    console.error("Erreur mise à jour statut carte: ", e);
                    showMessageModal("Erreur ❌", "Impossible de mettre à jour le statut.");
                }
            }
        };
        ...
        function initFirebase() {
            try {
                ...
                .then(() => {
                    document.getElementById('user-status').innerHTML = `<span class="status-indicator text-white">Connecté</span>`;
                ...
                .catch(error => {
                    ...
                    document.getElementById('user-status').innerHTML = `<span class="text-red-500">Déconnecté</span>`;
                });
                ...
                document.body.innerHTML = `
                    <div class="flex items-center justify-center min-h-screen text-center bg-red-800 text-white p-8">
                        <div>
                            ...
                            <h1 class="text-4xl font-bold mb-4">Problème Technique Caisse</h1>
                            <p class="text-xl mb-6">
                                L'application Caisse n'a pas pu démarrer. Veuillez contacter le support.
                            </p>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            menu.forEach(item => {
                ...
                button.innerHTML = `${item.icon} ${item.name} <span class="text-sm font-normal ml-1">(${item.price.toFixed(2)}€)</span>`;
...
<body class="p-4 md:p-8 flex flex-col lg:flex-row space-y-8 lg:space-y-0 lg:space-x-8">
    ...
    <div class="flex-1">
        ...
        <div id="cuisine-list">
            <div class="text-center text-white opacity-60 py-8"><i class="text-6xl mb-4">⏳</i><p>Chargement des commandes...</p></div>
        </div>
    </div>
    ...
    <div id="caisse-cart" class="lg:w-96 glass-morphism p-6 rounded-2xl shadow-2xl relative">
        ...
        <div class="flex justify-between items-center py-3 border-t border-b border-white border-opacity-30">
            ...
            <span id="caisse-total-before-discount" class="text-xl font-bold text-gray-300 line-through hidden">0.00€</span>
        </div>
        <div class="flex justify-between items-center py-3 mb-4">
            <span class="text-2xl font-black text-white">Total à Payer:</span>
            <span id="caisse-cart-total" class="text-3xl font-black text-white">0.00€</span>
        </div>
        ...
        <div class="flex space-x-3 mb-4">
            <input type="text" id="caisse-client-telephone" placeholder="Tél. Client (Recherche)" 
                   class="flex-1 p-3 border-0 rounded-xl bg-white bg-opacity-20 text-white placeholder-white placeholder-opacity-70 focus:bg-opacity-30 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50 transition-all">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
            <input type="text" id="caisse-client-prenom" placeholder="Prénom (si nouveau)" 
    ...
    <div id="split-payment-modal" class="fixed inset-0 bg-black bg-opacity-50 z-[50] flex items-center justify-center hidden" data-total="0.00" data-source="" data-orderid="">
        <div class="glass-dark rounded-2xl shadow-2xl w-full max-w-md m-4">
            <div class="bg-gradient-to-r from-purple-500 to-indigo-600 p-4 text-white font-bold rounded-t-2xl">Paiement Fractionné</div>
            <div class="p-6">
                <div class="text-center mb-6 p-4 rounded-xl border border-white border-opacity-30 bg-white bg-opacity-10">
                    <div class="text-lg font-bold text-white mb-2">Total de la commande: <span id="split-total-amount" class="text-3xl font-black ml-1">0.00€</span></div>
                    <div class="text-lg font-bold text-white">Reste à payer: <span id="split-remaining-amount" class="text-3xl font-black ml-1 text-red-500">0.00€</span></div>
                </div>
                ...
                <div class="flex space-x-3 mb-4">
                    <select id="split-payment-type" class="p-3 border-0 rounded-xl bg-white bg-opacity-20 text-white focus:ring-2 focus:ring-white focus:ring-opacity-50 transition-all flex-1">
                        <option value="Especes" class="bg-gray-700">💵 Espèces</option>
                        <option value="Carte" class="bg-gray-700">💳 Carte</option>
                        <option value="TicketResto" class="bg-gray-700">🎫 Ticket Resto</option>
                    </select>
    ...
    <div id="ticket-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
        <div class="glass-morphism rounded-2xl shadow-2xl w-full max-w-sm m-4">
            <div class="bg-gradient-to-r from-purple-500 to-pink-600 p-4 text-white font-bold rounded-t-2xl flex justify-between items-center">
                <span>🎫 Ticket de Caisse (<span id="qrcode-numero">#N/A</span>)</span><button id="close-ticket-btn" class="text-white hover:text-gray-200">✕</button>
            </div>
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-gray-600 mt-2 mb-4">Ce QR Code contient les détails du ticket.</p>
                <button onclick="window.downloadTicket()" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded-xl transition-all">Télécharger le TXT</button>
            </div>
        </div>
    </div>
</body>
